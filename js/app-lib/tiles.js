...
LIB.foodTile    = (x, y) => [x, y, LIB.pickRandom(["🍏","🍊","🍌","🍉","🍇","🍓","🌽","🍖"]), "#fffbd3", {
        ...
        think:function(){this.food -= 0.2; if(this.food < 0){this.die(); }},
        contStyle: "border: 1px solid burlywood"
    }];
LIB.wallTile    = (x, y) => [x, y, LIB.randOutOf(1,10)?LIB.pickRandom(["🌾","🌱"]):" ", "steelblue", {type:"wall", paintable: true, contStyle:"border: 1px solid #4070a0"}];

// map generation
LIB.generateLand = (scope) => {
    const tempMap = new Array(CONF.x).fill("").map(o=>({}));
    const tileWrapper = LIB.tileWrapper(scope);
    for (let i = 0; i < CONF.x * CONF.wallFactor; i++) {
        const x = LIB.rand(CONF.x - 1);
        const y = LIB.rand(CONF.y - 1);
        tempMap[x][y] = true;
    }

    tempMap.iterateKeys(x => {
        tempMap[x].iterateKeys(y => {
    ...
    for(let i = 0; i<CONF.treesPerSquareM*CONF.x*CONF.y; i++){
